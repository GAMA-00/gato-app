...
-- Primero, verificar y corregir la función submit_provider_rating
...
BEGIN
  ...
  -- Update provider's average rating in the users table
  ...
  FROM provider_avg
  WHERE users.id = p_provider_id AND provider_avg.rating_count > 0;
END;
$$;

-- Función para recalcular todos los promedios existentes
...
BEGIN
  ...
  LOOP
    -- Actualizar el promedio en la tabla users
    UPDATE users
...
-- Ejecutar la función para recalcular todos los promedios

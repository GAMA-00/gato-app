...
BEGIN
  ...
  WHILE iter_date <= p_end_date LOOP
    ...
    IF current_day_name = ANY(p_days_of_week) THEN
      ...
      slot_start_time := p_start_time;
      
      WHILE slot_start_time < p_end_time LOOP
        slot_end_time := slot_start_time + (p_slot_duration_minutes || ' minutes')::INTERVAL;
        
        -- No crear slot si el tiempo de fin excede el tiempo de trabajo
        IF slot_end_time <= p_end_time THEN
          -- Crear timestamps completos con zona horaria de Costa Rica
          slot_datetime_start := (iter_date + slot_start_time) AT TIME ZONE 'America/Costa_Rica';
          slot_datetime_end := (iter_date + slot_end_time) AT TIME ZONE 'America/Costa_Rica';
        ...
        slot_start_time := slot_start_time + (p_slot_duration_minutes || ' minutes')::INTERVAL;
      END LOOP;
    END IF;

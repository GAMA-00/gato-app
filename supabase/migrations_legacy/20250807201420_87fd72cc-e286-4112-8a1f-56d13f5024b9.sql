-- Atomic appointment creation + slot reservation with idempotency and cleanup
-- 1) Function: create_appointment_with_slot
-- 2) One-time cleanup of ghost slots (reserved without active appointment)

-- Create atomic RPC to create appointment and reserve slot in one transaction
...
-- One-time cleanup: release ghost slots (reserved but without active appointment)
-- We only touch non-recurring blocks to avoid interfering with recurring reservations
UPDATE provider_time_slots pts
SET is_available = true,
    is_reserved = false
WHERE is_reserved = true
  AND COALESCE(recurring_blocked, false) = false
  AND NOT EXISTS (
    SELECT 1 FROM appointments a
    WHERE a.provider_id = pts.provider_id
      AND a.listing_id = pts.listing_id
      AND a.start_time = pts.slot_datetime_start

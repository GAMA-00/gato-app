-- Fix ambiguous "status" reference in RPC used for multi-unit bookings
-- Ensure column references are qualified to avoid collision with OUT parameter "status"
...
BEGIN
  -- Log function call
  RAISE LOG 'create_appointment_with_slot called with provider_id: %, listing_id: %, start_time: %, end_time: %, selected_slot_ids: %', 

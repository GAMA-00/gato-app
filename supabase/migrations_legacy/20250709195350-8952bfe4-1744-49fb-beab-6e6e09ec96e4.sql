-- Fix extract() with explicit timestamp casting
...
BEGIN
  ...
  IF NEW.recurrence IN ('weekly', 'biweekly', 'monthly') AND 
    ...
    -- Create explicit timestamp for extract operations
    local_timestamp := (NEW.start_time AT TIME ZONE 'America/Costa_Rica')::TIMESTAMP;
    ...
    IF existing_rule_id IS NULL THEN
      -- Calculate day of week and day of month using explicit timestamp
      day_of_week_val := EXTRACT(DOW FROM local_timestamp)::INTEGER;
      day_of_month_val := EXTRACT(DAY FROM local_timestamp)::INTEGER;

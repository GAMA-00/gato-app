-- Fix triweekly and other recurrence slot blocking logic
...
BEGIN
  CASE recurrence_type
    WHEN 'weekly' THEN
      ...
      WHILE EXTRACT(DOW FROM next_date) != day_of_week OR next_date <= current_date LOOP
    ...
    WHEN 'biweekly' THEN
      ...
      weeks_diff := EXTRACT(DAYS FROM next_date - original_date)::INTEGER / 7;
...
BEGIN
  ...
  WHILE iter_date <= end_date LOOP
    ...
    DECLARE
      is_valid_occurrence BOOLEAN := false;
    ...
    BEGIN
      CASE appointment_record.recurrence
        ...
        WHEN 'biweekly' THEN
          weeks_from_original := EXTRACT(DAYS FROM iter_date - original_appointment_date)::INTEGER / 7;
    ...
    RAISE LOG 'Blocked slot on % at % (weeks from original: %)', 
      ...
      EXTRACT(DAYS FROM iter_date - original_appointment_date)::INTEGER / 7;

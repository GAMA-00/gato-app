...
BEGIN
  ...
  -- Delete existing slots for this date to regenerate cleanly
  DELETE FROM provider_time_slots
  WHERE provider_id = listing_record.provider_id
    AND listing_id = p_listing_id
    AND slot_date = p_target_date
    AND is_reserved = false; -- Keep reserved slots
  ...
  LOOP
    ...
    WHILE current_time + (service_duration || ' minutes')::INTERVAL <= slot_end_time LOOP
      slot_datetime_start := (p_target_date + current_time) AT TIME ZONE 'America/Costa_Rica';
      slot_datetime_end := (p_target_date + current_time + (service_duration || ' minutes')::INTERVAL) AT TIME ZONE 'America/Costa_Rica';
      ...
      IF slot_datetime_start > NOW() THEN
        ...
        IF NOT EXISTS (
          ...
          INSERT INTO provider_time_slots (
            ...
            p_target_date,
            current_time,
            current_time + (service_duration || ' minutes')::INTERVAL,
            slot_datetime_start,
            slot_datetime_end,
            true,
            false,
      ...
      -- Move to next slot
      current_time := current_time + (service_duration || ' minutes')::INTERVAL;
    END LOOP;
  END LOOP;
  ...
  RETURN QUERY SELECT 'SUCCESS'::TEXT, 
    format('Regenerated slots for %s. Created %s new slots.', p_target_date, slots_created)::TEXT, 
...
-- Execute the diagnosis and fix for the current listing (Friday August 8th, 2025)

-- Search for functions that use extract() and might be causing the issue
-- Let's fix all the functions that use extract() with wrong parameter types

-- First, let's fix the recurring rules function that creates appointments
...
BEGIN
  ...
  IF NEW.recurrence IN ('weekly', 'biweekly', 'monthly') AND 
    ...
    IF existing_rule_id IS NULL THEN
      -- Calculate day of week and day of month using LOCAL time - FIX THE EXTRACT ISSUE
      day_of_week_val := EXTRACT(DOW FROM (NEW.start_time AT TIME ZONE 'America/Costa_Rica')::TIMESTAMP)::INTEGER;
      day_of_month_val := EXTRACT(DAY FROM (NEW.start_time AT TIME ZONE 'America/Costa_Rica')::TIMESTAMP)::INTEGER;
...
CREATE TRIGGER create_recurring_rule_from_appointment_trigger

-- Update handle_new_user function to properly handle provider residencias
...
BEGIN
  ...
  RAISE LOG 'Creating user with metadata: name=%, email=%, phone=%, role=%, condominium_name=%, house=%, residencia=%', 
    user_name, NEW.email, user_phone, default_role, actual_condominium_name, user_house_number, user_residencia_id;
  ...
  BEGIN
    ...
    RAISE LOG 'User inserted successfully with ID: % and condominium: %', NEW.id, actual_condominium_name;

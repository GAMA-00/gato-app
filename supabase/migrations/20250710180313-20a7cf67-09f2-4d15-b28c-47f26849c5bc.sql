...
BEGIN
  ...
  iter_date := GREATEST(rule_record.start_date, CURRENT_DATE);
  end_date := iter_date + (p_months_ahead * 30); -- Aproximadamente 12 meses
  ...
  WHILE iter_date <= end_date LOOP
    ...
    BEGIN
      ...
      CASE rule_record.recurrence_type
        WHEN 'weekly' THEN
          IF EXTRACT(DOW FROM iter_date::TIMESTAMP)::INTEGER = rule_record.day_of_week THEN
            should_block := true;
          END IF;
        WHEN 'biweekly' THEN
          IF EXTRACT(DOW FROM iter_date::TIMESTAMP)::INTEGER = rule_record.day_of_week AND
             EXTRACT(DAYS FROM iter_date - rule_record.start_date)::INTEGER % 14 = 0 THEN
            should_block := true;
          END IF;
        WHEN 'monthly' THEN
          IF EXTRACT(DAY FROM iter_date::TIMESTAMP)::INTEGER = rule_record.day_of_month THEN
            should_block := true;
          END IF;
      END CASE;
    ...
    CASE rule_record.recurrence_type
      WHEN 'weekly' THEN
        iter_date := iter_date + 7;
      WHEN 'biweekly' THEN
        iter_date := iter_date + 14;
      WHEN 'monthly' THEN
        iter_date := iter_date + 30; -- Aproximación para encontrar el siguiente mes
    END CASE;
  END LOOP;
